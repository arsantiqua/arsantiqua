/*
 * cs4398.c  --  CS4398 ALSA Soc Audio codec driver
 *
...
static const struct reg_default cs4398_reg_defaults[] = {
	...
	{ 4, 0xC1 },	/* r04	- Mute Control */
	{ 5, 0x00 },	/* r05	- Channel A Volume Control */
	{ 6, 0x00 },	/* r06	- Channel B Volume Control */
	{ 7, 0xB0 },	/* r07	- Ramp and Filter Control */
	{ 8, 0x80 },	/* r08	- Misc. Control 0 */
	{ 9, 0x08 },	/* r09	- Misc. Control 1 */
};
...
{
	switch (reg) {
	case CS4398_CHIPID ... CS4398_MISC2:
		return true;
	default:
		return false;
	}
...
static int cs4398_digital_mute(struct snd_soc_dai *dai, int mute)
{
	struct snd_soc_codec *codec = dai->codec;
	int reg;
	...
	return snd_soc_update_bits(codec, CS4398_MUTE, MUTE_AB_MASK, reg);
}
...
static const struct snd_soc_dai_ops cs4398_dai_ops = {
	...
	.digital_mute	= cs4398_digital_mute,
};
				      ...
				      const struct i2c_device_id *id)
{
	...
	/* Reset the Device */
	cs4398->reset_gpio = devm_gpiod_get_optional(&client->dev,
		"reset", GPIOD_OUT_LOW);
	if (IS_ERR(cs4398->reset_gpio))
		return PTR_ERR(cs4398->reset_gpio);
...
MODULE_AUTHOR("Dennis Fleming <arsantiqua@sbcglobal.net>");
